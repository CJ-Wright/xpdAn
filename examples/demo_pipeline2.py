...
def outputs(mean, q, buffer_size=10, **kwargs):
    # integrated intensities
    out_mean = mean.combine_latest(q, emit_on=0).SimpleToEventStream(
        ("iq", "q"),
    )
    return locals()
...
def buffer_gather(out_mean, buffer_size=10, **kwargs):
    out_mean = out_mean.buffer(buffer_size).gather()
