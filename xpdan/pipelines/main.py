...
from xpdtools.pipelines.xpd_raw_pipeline import *
...
# Get foreground
FromEventStream(source, 'event', ('seq_num',), stream_name='seq_num'
                ).connect(img_counter)
...
for kwargs, node in zip([mask_kwargs, mask_kwargs, mask_kwargs,
                         fq_kwargs, pdf_kwargs], [all_mask, first_mask,
                                                  no_mask,
                                                  fq, pdf]):
...
start_yaml_string.map(clean_template).zip(start_docs).map(dump_yml)
...
(q.zip(mean).zip(filename_name_nodes['iq_name'])
    .map(lambda l: (*l[0], l[1]))
    .starsink(lambda x, y, n: save_output(x, y, n, 'Q'),
              stream_name='save integration {}'.format('Q')))
(tth.zip(mean).zip(filename_name_nodes['tth_name'])
    .map(lambda l: (*l[0], l[1]))
...
# PDF
(pdf.zip_latest(filename_name_nodes['pdf_name']).map(lambda l: (*l[0], l[1]))
    .starsink(lambda r, gr, config, name: pdf_saver(r, gr, name, config),
              stream_name='pdf saver'))
# F(Q)
(fq.zip_latest(filename_name_nodes['fq_name']).map(lambda l: (*l[0], l[1]))
    .starsink(lambda r, gr, config, name: pdf_saver(r, gr, name, config),
              stream_name='fq saver'))
# S(Q)
(sq.zip_latest(filename_name_nodes['fq_name']).map(lambda l: (*l[0], l[1]))
    .starsink(lambda r, gr, config, name: pdf_saver(r, gr, name, config),
